...
const Administration = () => {
  ...
  return (
    <div className="chart">
      ...
      <p>Options d'administration pour gérer les utilisateurs et les paramètres.</p>
      <div>
        <input
          type="text"
        ...
        />
        {editingGroupe ? (
          <button onClick={saveGroupe}>Sauvegarder</button>
        ) : (
        ...
        <tbody>
          {groupes.map((groupe) => (
            <tr key={groupe.id}>
              ...
              <td>
                <button onClick={() => editGroupe(groupe)}>Éditer</button>
                <button onClick={() => deleteGroupe(groupe)}>Supprimer</button>
              </td>
            </tr>
          ))}
        </tbody>
